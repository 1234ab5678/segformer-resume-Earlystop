...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------#
    #   resume             是否进行断点续训
    #   resume_model       断点续训模型的路径
    # ------------------------------------------------------------------#
    resume              = True
    resume_model        = './logs/loss_2023_10_25_19_54_03/last_epoch_weights.pth'
    # ------------------------------------------------------------------#
    #   earlystop             是否进行提前终止
    #   earlystop_patience    提前终止的迭代阈值
    #   终止阈值在数据集较小时需要设置的大一些，在大数据集下可以设置的小一些
    # ------------------------------------------------------------------#
    earlystop           = True
    ...
    #------------------------------------------------------------------#
    VOCdevkit_path  = '/home/tableai/Y.CL/Project/TableGPT/TableRecognition/StructureRecognition/Algorithms/RowColExtraction/RowColSegmentation/Dataset/RowSegmentation/'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    if local_rank == 0:
        if not resume:
            ...
            loss_history = LossHistory(log_dir, model, input_shape=input_shape, resume=resume)
            save_dir = log_dir
        else:
            # time_str = datetime.datetime.strftime(datetime.datetime.now(), '%Y_%m_%d_%H_%M_%S')
            log_dir = os.path.join(save_dir, resume_model.split('/')[2])
            # os.path.join(save_dir, "loss_" + str(time_str))
            loss_history = LossHistory(log_dir, model, input_shape=input_shape, resume=resume)
            save_dir = log_dir
    else:
        loss_history = None
    ...
    if True:
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines, VOCdevkit_path, log_dir, Cuda, resume, \
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if resume:
                # patience = earlystop_patience
                with open(os.path.join(save_dir, "earlystop_counter.txt"), 'r') as f:
                    counters = f.readlines()
                with open(os.path.join(save_dir, "val_loss_min.txt"), 'r') as f:
                    val_loss_mines = f.readlines()
                for i in range(len(val_loss_mines)):
                    val_loss_mines[i] = float(val_loss_mines[i])

                # print(int(counters[-1]))
                # print(np.min(val_loss_mines))
                # earlystopping.counter = counters[-1]
                earlystopping = EarlyStopping(patience, counter=int(counters[-1]), val_loss_min=(-1)*np.max(val_loss_mines), best_score=np.max(val_loss_mines), verbose=True)

            flag = fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, \
                dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, resume, earlystopping, local_rank)
            if flag:
                break
